...
describe('SSO Azure B2B guest detection and rejection', () => {
  ...
  describe('isExternal persistence', () => {
    ...
    it('persists isExternal when linking existing user to Azure OID', () => {
      // Slice #67 (Bundle A slice 2): isExternal persistence moved out of the
      // inline `db.update(users).set({...})` and into the `flipIsExternal`
      // helper so that flipping the flag also revokes any pre-flip sessions +
      // refresh-token families. The invite-claim branch updates the OID + name
      // separately, then calls flipIsExternal.
      expect(ssoSource).toMatch(/db\.update\(users\)\.set\(\{[^}]*externalId:\s*oid[^}]*\}\)/s);
      expect(ssoSource).toMatch(/flipIsExternal\(user\.id,\s*isExternal\)/);
    });

    it('persists isExternal on returning-user sync', () => {
      // Slice #67: the returning-user branch's bulk update no longer carries
      // isExternal — flipIsExternal owns the flag write. The bulk set keeps
      // the non-flag fields (name + email + optional lang) and is followed by
      // a flipIsExternal call.
      const syncUpdates = ssoSource.match(/db\s*\.update\(users\)\s*\.set\(\{[^}]*\}\)/gs) || [];
      const hasSyncWithEmail = syncUpdates.some(
        (block) => block.includes('email') && !block.includes('isExternal'),
      );
      expect(hasSyncWithEmail).toBe(true);
      // flipIsExternal must be present in the same file (the returning-user
      // branch and the invite-claim branch share the import + the call site).
      expect(ssoSource).toMatch(/flipIsExternal\(user\.id,\s*isExternal\)/);
    });
  });
