...
// Actor builders are owned by `services/auth`. Re-exported here as the
// canonical lifecycle public surface; `SYSTEM_ACTOR` is aliased to the legacy
// lowercase `systemActor` name so existing callers continue to compile.
// Deep imports rather than the auth barrel — the barrel transitively
// evaluates `flipIsExternal`'s production wiring (which imports `db`),
// which would force every lifecycle-importing test to mock `db` too.
