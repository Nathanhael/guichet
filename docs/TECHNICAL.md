...
- **Azure B2B Guests**: Partner employees can be invited as B2B guests in our Azure tenant and authenticate via their home IdP. The SSO callback detects them from the `acct === 1` or `idp` claim and sets `users.is_external`. Guests are enforced single-partner (fail-closed audit at login), blocked from destructive partner-admin mutations via the `destructive_admin` capability check (`services/auth/capabilities.ts`), and blocked from internal-only PII reads (e.g. the admin roster) via the `audit_read` capability check. Both gates are resolved inline through `trpcActor(ctx, { capability })` — no shared procedure wrapper. A `GUEST` badge surfaces in the UI. Full details in `docs/TENANT_IDENTITY_SPEC.md`.
...
- **Uploads Tenant-Isolation Gate**: The `/uploads/<filename>` proxy (`server/middleware/uploadProxy.ts`) verifies both the JWT cookie AND that the requested file belongs to the caller's partner before streaming bytes. Filename → owning `partnerId` lookup goes through `services/uploadOwnership.lookupFilePartnerId`, which joins `messages.attachments` (JSONB containment, GIN-indexed) and legacy `messages.mediaUrl` against `tickets.partnerId`. Outcomes: missing JWT → 401, filename not registered in any message → 404, foreign-tenant filename → 403, match → 200. Platform operators carry a partner-scoped JWT after `/enter-partner`, so no special branch is needed; cross-partner access from `PlatformView` flows through `platform.*` endpoints which resolve files server-side and bypass the proxy.
- **Uploads EXIF / GPS Strip**: `services/exifStrip.stripImageMetadata` re-encodes JPEG / PNG / WebP uploads through sharp's default pipeline before they land in storage, dropping EXIF (incl. GPS lat/lng), ICC, XMP, IPTC, and camera-fingerprint fields. `.rotate()` bakes the EXIF orientation into pixels first so portrait phone photos don't flip after the strip. Re-encode also caps width at `IMAGE_MAX_DIMENSION` (default 2000, env-overrideable) — phone-camera 4K is wasted on helpdesk screenshots and inflates storage. Non-image MIMEs (PDF, Office, CSV, text) pass through unchanged; document metadata stripping is tracked as a follow-up.
...
- **Per-Tenant Configuration**: Each partner has `aiEnabled` flag and `aiFeatures` JSONB controlling which AI capabilities are active (message improvement, translation, voice transcription, canned-response translation). Platform admins toggle features in the Edit Partner modal.
- **Available Actions** (`AiAction`): `improve` (message rewriting, role-aware), `translate` (per-message between nl/en/fr, source-language auto-detected), `transcribe` (voice dictation via the dedicated `/api/v1/ai/transcribe` Express route), `classify`, `suggest`, `match_canned`. Summarization is **not** a current action — chat summary / auto-summarize-on-close were removed.
- **Message Improvement**: Role-aware rewriting — agents get clarity-focused rewrites, support gets actionable step-by-step rewrites. Optional or forced modes with revert-to-original (`ImproveDiffModal`).
- **Translation**: Per-message translation between nl/en/fr via `ai.translateMessage`. Caching + skip rules in `services/ai/translateCache.ts` and `translateGuards.ts`.
- **PII Redaction & Audit Verbosity**: `piiRedaction.ts` strips PII before prompts leave the server. Per-partner audit verbosity (`hash` / `metadata` / `full`) governs how much of the prompt/response is persisted in `ai_usage_log`.
...
- **Window Event Architecture**: To avoid deep prop-drilling or complex handle-passing for UI modals, the system uses a producer/consumer pattern based on `window` CustomEvents. The keyboard-shortcuts hook and the support `CommandPalette` dispatch events (e.g., `support:open-label-picker`, `support:open-palette`), which are consumed by the component owning the modal state (`ChatHeader`, `SupportView`).
