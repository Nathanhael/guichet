...
- **Tenant Mapping**: The current platform treats `partners` as tenants and `memberships` as the authorization link from internal users to one or more tenants.
...
- **Bounded Invite Claim Window**: SSO-provisioned invite rows expire after `INVITE_TTL_DAYS` (currently 7) — older rows are ignored at the SSO callback. Guards against stale mailed-invite replay.
