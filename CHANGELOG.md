...
- **Guichet rebrand** — full sweep renaming Tessera → Guichet across code, config, docs, cookie names (`guichet_token`, `guichet_refresh`), Docker compose project, Postgres DB name, Grafana dashboard, git remote.
...
- **Minimal seed** — replaced the prior flag matrix and full faker demo dataset with a single minimal seed: one partner (`acme`), 6 named users, 6 hand-written tickets. Easier to reason about locally.
