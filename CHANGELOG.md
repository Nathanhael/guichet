...
- **GUEST badge (brutalist, amber)** — rendered next to display names in `UserMenu` (own identity), `AdminTeam` (team rows), and `SidebarFooter` (support team panel). Built from tokens (`accent-amber` outline + mono uppercase), no radius, no shadow. `trpc.user.me` added as the canonical current-user query; `trpc.status.getTeamStatus` batch-enriches with `isExternal` for GUEST-badge rendering in team lists. Follow-up: ChatHeader participants + MessageBubble senders still need `isExternal` plumbing through ticket participants + message payloads.
