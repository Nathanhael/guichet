...
test.describe('Agent View', () => {
  ...
  test('agent view loads without errors', async ({ page }) => {
    // The previous regex `/error|crash|500/i` matched incidental message-body
    // text in agent_julie's seeded ticket history (e.g. messages mentioning
    // those words in conversation), producing false positives. Tighten the
    // assertion to actual error-UI surfaces only:
    //   - ErrorBoundary fallback text: "Component failed to load"
