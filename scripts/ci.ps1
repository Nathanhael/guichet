...
#        powershell -File scripts/ci.ps1 -Skip lint    (skip a step)

param(
    [ValidateSet("typecheck", "tenant-isolation-guard", "lint", "audit", "test-server", "test-client", "migrate", "build")]
    [string[]]$Skip = @()
)
...
# Vite production build runs in its own step so Rolldown/Vite regressions fail
# CI even when other test suites pass.
