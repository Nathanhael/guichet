...
export default function ExpertPerformance({ expertStats, className = '' }) {
    const [hoveredExpert, setHoveredExpert] = React.useState(null);
    const [activeExpert, setActiveExpert] = React.useState(null);

    if (!expertStats || expertStats.length === 0) {
        return (
            <Panel title="Expert Activity Trend" className={className}>
    ...
    const dateMap = {};
    const expertNames = expertStats.map(e => e.name);

    expertStats.forEach(expert => {
        expert.trend?.forEach(t => {
            if (!dateMap[t.date]) {
            ...
            dateMap[t.date][expert.name] = t.count;
        });
    });
    ...
    return (
        <Panel title="Expert Activity Trend" badge="Productivity" className={`flex flex-col ${className}`}>
                ...
                <div className="relative">
                    <div className="flex flex-wrap justify-end gap-2 mb-4">
                        {expertNames.map((name, index) => {
                            ...
                            const isFocused = activeExpert === name || hoveredExpert === name;
                            return (
                                <button
                                    key={name}
                                    onClick={() => setActiveExpert(activeExpert === name ? null : name)}
                                    onMouseEnter={() => setHoveredExpert(name)}
                    ...
                    <div className="h-[280px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }} onClick={() => setActiveExpert(null)}>
                                ...
                                {expertNames.map((name, index) => {
                                    const isFocused = activeExpert === name || hoveredExpert === name;
                                    const isAnyActive = activeExpert !== null || hoveredExpert !== null;
            ...
            <div className="mt-4 text-[10px] text-slate-400 font-medium italic flex justify-between border-t border-white/5 pt-3">
                <span>Click an expert to lock focus, click background to reset</span>
                <span>Period activity per expert</span>
            </div>
        </Panel>
    );
}
