...
export default function LabelManager() {
    ...
    const addLabel = async () => {
        if (!newText.trim() || !newColor) return;
        setError(null);
        try {
    ...
    const deleteLabel = async (id: string) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setError(data.error || `Failed to delete label: ${res.status}`);
            }
        } catch (err: any) {
            setError(`Network error: ${err.message}`);
        }
        ...
        <div className="space-y-6 max-w-4xl mx-auto animate-fade-in">
            ...
            <div className="glass-card p-6 shadow-soft">
                ...
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <button
                        onClick={addLabel}
                        disabled={!newText.trim() || !newColor}
                        className="bg-brand-500 hover:bg-brand-600 text-white px-6 py-2 rounded-xl text-sm font-bold transition-all disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed shadow-md shadow-brand-500/20"
                    >
                        Add Label
                    </button>
                </div>
                    ...
                    labels.map(l => (
                        <div key={l.id} className="bg-white dark:bg-brand-800 rounded-2xl border border-gray-100 dark:border-brand-700 p-4 flex items-center justify-between group hover:shadow-md transition-all animate-slide-up">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-bold text-gray-800 dark:text-gray-100">{l.name}</span>
                            </div>
                            <button
