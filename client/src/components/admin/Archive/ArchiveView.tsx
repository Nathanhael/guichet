...
export default function ArchiveView() {
    ...
    const [preview, setPreview] = useState<ArchiveTicket | null>(null);
    ...
    function fetchTickets({ reset = false, s = search, d = dept, from = dateFrom, to = dateTo, off = 0 } = {}) {
        ...
        fetch(`/api/tickets?${params}`, {
            ...
            .then((r) => r.json())
            .then(({ tickets: rows, total: tCount }: { tickets: ArchiveTicket[], total: number }) => {
    ...
    function duration(tRef: ArchiveTicket) {
        if (!tRef.closedAt || !tRef.createdAt) return '—';
        const m = Math.round((new Date(tRef.closedAt).getTime() - new Date(tRef.createdAt).getTime()) / 60000);
