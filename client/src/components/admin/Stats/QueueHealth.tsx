...
export default function QueueHealth({ stats }: QueueHealthProps) {
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <div className="flex justify-between items-end mb-1">
                            ...
                            <span className={`text-xs font-bold ${(stats.deptSla?.['DSC'] ?? 0) >= 90 ? 'text-emerald-500' : 'text-rose-500'}`}>{stats.deptSla?.['DSC'] ?? 0}%</span>
                        </div>
                        <div className="h-1 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div className="h-full bg-purple-500" style={{ width: `${stats.deptSla?.['DSC'] ?? 0}%` }} />
                        </div>
                    </div>
                    <div>
                        <div className="flex justify-between items-end mb-1">
                            <span className="text-[10px] font-bold text-teal-600">FOT</span>
                            <span className={`text-xs font-bold ${(stats.deptSla?.['FOT'] ?? 0) >= 90 ? 'text-emerald-500' : 'text-rose-500'}`}>{stats.deptSla?.['FOT'] ?? 0}%</span>
                        </div>
                        <div className="h-1 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                            <div className="h-full bg-teal-500" style={{ width: `${stats.deptSla?.['FOT'] ?? 0}%` }} />
                        </div>
                    </div>
                </div>
            </div>
