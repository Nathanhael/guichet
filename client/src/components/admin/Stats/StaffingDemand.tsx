import { ResponsiveContainer, ComposedChart, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ReferenceArea, Cell } from 'recharts';
...
export default function StaffingDemand({ hourlyStaffing, activeHour, onHourClick, className = '' }: StaffingDemandProps) {
  ...
  const chartData: StaffingData[] = hourlyStaffing.map(item => ({
    ...item,
    time: formatHour(item.hour),
    loadFactor: item.experts > 0 ? (item.tickets / item.experts).toFixed(1) : (item.tickets > 0 ? '∞' : '0.0'),
  ...
  const coveragePercent = Math.min(100, Math.round((businessHoursWithStaff / 16) * 100)); // 07:00-22:00 is 16 hours
  ...
  const getEfficiency = () => {
    ...
    if (totalTickets === 0) return { label: 'No Data', color: 'text-slate-400', bg: 'bg-slate-500/10' };
    
    const criticalHours = chartData.filter(h => h.isBusinessHour && h.tickets > 0 && h.experts === 0).length;
    const avgLoad = activeHours.length > 0
        ? activeHours.reduce((a, b) => a + (b.loadFactor === '∞' ? 20 : parseFloat(b.loadFactor)), 0) / activeHours.length
        : 0;
    ...
    const maxLoad = Math.max(...chartData.map(h => h.loadFactor === '∞' ? 20 : parseFloat(h.loadFactor)));
    if (maxLoad > 8) return { label: 'High Tension', color: 'text-amber-500', bg: 'bg-amber-500/10', desc: 'Experts handling >8 tickets at peak.' };
  ...
  return (
    <Panel title="Staffing vs Demand Analysis" badge="Hourly Supply/Demand" className={`flex flex-col ${className}`}>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <ReferenceArea
              ...
              label={{ position: 'top', value: 'Service Window (07:30-22:30)', fill: '#6366f1', fontSize: 10, fontWeight: 'bold' }}
            />
            ...
            <YAxis
              ...
              axisLine={false}
              tickLine={false}
              tick={{ fontSize: 10, fill: '#818cf8', fontWeight: 600 }}
              label={{ value: 'Questions', angle: -90, position: 'insideLeft', offset: 15, fontSize: 10, fill: '#818cf8' }}
            />
            <YAxis
              yAxisId="right"
              orientation="right"
              axisLine={false}
              tickLine={false}
              tick={{ fontSize: 10, fill: '#f472b6', fontWeight: 600 }}
              label={{ value: 'Experts', angle: 90, position: 'insideRight', offset: 15, fontSize: 10, fill: '#f472b6' }}
            />
            <Tooltip
              content={({ active, payload, label }) => {
                if (!active || !payload || !payload.length) return null;
                const data = payload[0].payload;
                const ratio = data.loadFactor === '∞' ? 20 : parseFloat(data.loadFactor);
            ...
            >
              {chartData.map((entry, index) => {
                const ratio = entry.loadFactor === '∞' ? 20 : parseFloat(entry.loadFactor);
                const getBaseColor = () => {
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-pink-50/5 dark:bg-pink-900/10 rounded-2xl border border-pink-500/10 text-center md:text-left">
          ...
          <div className="flex items-baseline justify-center md:justify-start gap-2">
            <p className="text-2xl font-black text-pink-600 dark:text-pink-400">
              {(activeHours.reduce((acc, curr) => acc + (curr.loadFactor === '∞' ? 20 : parseFloat(curr.loadFactor)), 0) / (activeHours.length || 1)).toFixed(1)}
            </p>
        ...
        <div className="p-4 bg-amber-50/5 dark:bg-amber-900/10 rounded-2xl border border-amber-500/10 text-center md:text-left">
          ...
          <p className="text-[10px] text-slate-500 mt-1">Window: 07:30-22:30.</p>
        </div>
      ...
      <div className="mt-6 p-3 bg-slate-50/5 dark:bg-slate-900/30 rounded-xl border border-dotted border-slate-300 dark:border-slate-700">
        ...
        <p className="text-center text-[9px] text-slate-500 mt-2 italic">
          Ratio = Tickets per Expert. Staffing values are <b>daily averages</b> for the selected period.
        </p>
      </div>
    </Panel>
  );
