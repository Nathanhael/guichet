...
interface StatsOverviewProps {
    stats: PeriodStats & {
        previousPeriod?: Partial<PeriodStats>;
    };
}
...
export default function StatsOverview({ stats }: StatsOverviewProps) {
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            ...
            <StatCard label="Response Time" value={stats.avgResponseMinutes > 0 ? `${stats.avgResponseMinutes}m` : '—'} color="gray" prev={(stats.previousPeriod?.avgResponseMinutes ?? 0) > 0 ? `${stats.previousPeriod?.avgResponseMinutes}m` : undefined} invertTrend />
            <StatCard label="Avg Duration" value={stats.avgDurationMinutes > 0 ? `${stats.avgDurationMinutes}m` : '—'} color="gray" prev={(stats.previousPeriod?.avgDurationMinutes ?? 0) > 0 ? `${stats.previousPeriod?.avgDurationMinutes}m` : undefined} invertTrend />
