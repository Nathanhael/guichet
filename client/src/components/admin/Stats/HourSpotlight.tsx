...
export default function HourSpotlight({ hourData, expertStats, className = '' }: HourSpotlightProps) {
    if (!hourData) {
        return (
            <Panel title="Hour Spotlight" className={`flex flex-col ${className}`}>
    ...
    const { hour, tickets, experts, slaHealth } = hourData;
    ...
    const getWorkloadInfo = (ratio: string) => {
    ...
    return (
        <Panel title={`Hour Spotlight: ${formatHour(hour)}`} className={`flex flex-col ${className}`}>
