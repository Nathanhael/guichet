...
export default function StaffingDemand({ hourlyStaffing, activeHour, onHourClick, className = '' }) {
    ...
    // ... (metrics calculation stays same)
    const activeHours = chartData.filter(h => h.experts > 0);
    const avgShiftSize = activeHours.length > 0
        ? (activeHours.reduce((acc, curr) => acc + curr.experts, 0) / activeHours.length).toFixed(1)
        : "0.0";
    ...
    const maxLoadHour = chartData.reduce((prev, current) => {
        return parseFloat(prev.loadFactor) > parseFloat(current.loadFactor) ? prev : current;
    }, chartData[0]);
    const getEfficiency = () => {
        ...
        const avgLoad = activeHours.reduce((a, b) => a + parseFloat(b.loadFactor), 0) / (activeHours.length || 1);
        ...
        if (maxLoadHour.loadFactor > 8) return { label: 'High Tension', color: 'text-amber-500', bg: 'bg-amber-500/10', desc: 'Experts handling >8 tickets at peak.' };
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <ReferenceArea
                            ...
                            label={{ position: 'top', value: 'Business Hours', fill: '#6366f1', fontSize: 10, fontWeight: 'bold' }}
                        />
                        ...
                        <Tooltip
                            content={({ active, payload, label }) => {
                                ...
                                const ratio = parseFloat(data.loadFactor);
                        ...
                        >
                            {chartData.map((entry, index) => {
                                const ratio = parseFloat(entry.loadFactor);
                                const getBaseColor = () => {
                ...
                <div className="p-4 bg-pink-50/5 dark:bg-pink-900/10 rounded-2xl border border-pink-500/10 text-center md:text-left">
                    ...
                    <div className="flex items-baseline justify-center md:justify-start gap-2">
                        <p className="text-2xl font-black text-pink-600 dark:text-pink-400">
                            {(activeHours.reduce((acc, curr) => acc + parseFloat(curr.loadFactor), 0) / (activeHours.length || 1)).toFixed(1)}
                        </p>
