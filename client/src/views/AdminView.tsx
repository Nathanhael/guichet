...
import { Statistics, FeedbackItem, Rating, User } from '../types';
...
export default function AdminView() {
  ...
  }, [token, statsDept, statsDateFrom, statsDateTo, excludeWeekends]);
  ...
  const markTreated = async (id: string) => {
    ...
    if (res.ok) setFeedback(prev => prev.map(f => f.id === id ? { ...f, treated: true } : f));
  };
  ...
  useEffect(() => {
    ...
    fetchStats();
  }, [statsDept, statsDateFrom, statsDateTo, excludeWeekends, fetchStats, token, setTickets]);
              ...
              {!stats ? <p className="text-slate-400">{t('loading')}</p> : (<>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="lg:col-span-1 flex flex-col gap-6 h-full">
                    ...
                    <ErrorBoundary>
                      <HourSpotlight
                        hourData={selectedHour !== null ? stats.hourlyStaffing.find(h => h.hour === selectedHour) || null : null}
