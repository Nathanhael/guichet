...
import ManagerArchive from '../components/manager/ManagerArchive';
import ManagerFeedback from '../components/manager/ManagerFeedback';
import ManagerLabels from '../components/manager/ManagerLabels';

type ManagerTab = 'dashboard' | 'tickets' | 'archive' | 'feedback' | 'labels';

export default function ManagerView() {
  const { user, logout } = useStore();
  const t = useT();
  const [view, setView] = useState<ManagerTab>('dashboard');

  if (!user) return null;

  const NavButton = ({ id, label, icon }: { id: ManagerTab; label: string; icon: React.ReactNode }) => (
    <button
      onClick={() => setView(id)}
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-transparent animate-fade-in">
      <nav className="bg-brand-900/95 backdrop-blur-md text-white px-6 py-3 flex items-center justify-between shadow-lg sticky top-0 z-50 border-b border-brand-800">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-gradient-to-r from-accent-500 to-rose-500 px-2.5 py-1 rounded-md font-semibold tracking-wide shadow-sm">Manager</span>
          </div>
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-brand-950 p-6 custom-scrollbar">
        {view === 'dashboard' && <ManagerStats />}
        {view === 'tickets' && <ManagerTickets />}
