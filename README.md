...
- **SSO-Only Authentication** — Azure Entra ID for all staff. No passwords, no MFA. Emergency access via the break-glass CLI (`server/scripts/break_glass.ts`).
...
- **Identity Provisioning** — Users surface automatically when an Azure SSO callback resolves them to a partner via group mappings. No passwords, no invite email. Removing a user from a partner revokes their sessions + refresh tokens immediately.
